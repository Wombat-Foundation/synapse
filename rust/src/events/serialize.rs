...
fn serialize_event_value(
    ...
    // Replace `age_ts` with `age`, with `age` calculated as the difference
    // between the current time and `age_ts`. This is an optional field in the
    // spec.
    //
    // We might not have an `age_ts`, e.g. if a remote server did not include
    // the `age` field in the event it sent us. Since `age_ts` is generated by
    // us, it *should* be an integer, but it is possible for it to be out of i64
    // range (e.g. if the original `age` was close to the maximum i64 value). In
    // that case, just omit `age` rather than erroring (otherwise a once valid
    // event could start failing).
    ...
    if config.include_admin_metadata {
        if event.internal_metadata.soft_failed()? {
            unsigned.insert(unsigned_field::SOFT_FAILED.to_owned(), Value::Bool(true));
        }
...
fn only_fields(dictionary: &Map<String, Value>, fields: &[String]) -> PyResult<Map<String, Value>> {
